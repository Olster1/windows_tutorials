...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hInstPrev, PSTR cmdline, int cmdshow) {
    ...
    while(running) {
        ...
        //NOTE: Left Mouse Button Press
        for(int i = 0; i < global_keyDownStates[PLATFORM_MOUSE_LEFT_BUTTON].pressedCount; ++i) {
            char buffer[256];
            sprintf(buffer, "Left Mouse Pressed %d\n", i);
            OutputDebugStringA(buffer);
        }

        //NOTE: Left Mouse Button Release
        for(int i = 0; i < global_keyDownStates[PLATFORM_MOUSE_LEFT_BUTTON].releasedCount; ++i) {
            char buffer[256];
            sprintf(buffer, "Left Mouse released %d\n", i);
            OutputDebugStringA(buffer);
        }

        //NOTE: Right Mouse Button Press
        for(int i = 0; i < global_keyDownStates[PLATFORM_MOUSE_RIGHT_BUTTON].pressedCount; ++i) {
            char buffer[256];
            sprintf(buffer, "Right Mouse Pressed %d\n", i);
            OutputDebugStringA(buffer);
        }

        //NOTE: Right Mouse Button Release
        for(int i = 0; i < global_keyDownStates[PLATFORM_MOUSE_RIGHT_BUTTON].releasedCount; ++i) {
            char buffer[256];
            sprintf(buffer, "Right Mouse released %d\n", i);
            OutputDebugStringA(buffer);
        }

        //NOTE: Left Mouse Button Down
        if(global_keyDownStates[PLATFORM_MOUSE_LEFT_BUTTON].isDown) {
            OutputDebugStringA("Left Mouse down\n");
        }

        //NOTE: Right Mouse Button Down
        if(global_keyDownStates[PLATFORM_MOUSE_RIGHT_BUTTON].isDown) {
